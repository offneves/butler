...
export async function getUserPlan(): Promise<PlanData | null> {
    const apiUrl = import.meta.env.VITE_APP_API_ENDPOINT || import.meta.env.APP_API_ENDPOINT || "http://localhost:8080";
    const token = localStorage.getItem("token");
    ...
    try {
        ...
        const response = await fetch(`${apiUrl}/user/${userId}/plan`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
        ...
        if (!response.ok) {
            throw new Error("Erro ao buscar dados do plano do usuário");
        }
